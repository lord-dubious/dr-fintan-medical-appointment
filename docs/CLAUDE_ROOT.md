...
- **Tech Stack**: Laravel, PHP, JavaScript, CSS (Blade for views)
- **Main File**: `routes/web.php` (for routing) and `app/Http/Controllers/` (for logic)
- **Core Mechanic**: Medical appointment scheduling and management, user role management (Admin, Doctor, Patient), and integrated video consultation via Daily.co.
- **Key Integration**: Daily.co (video calls), Paystack (payment processing).
- **Platform Support**: Web application, designed for standard browser access.
- **DO NOT**: Directly modify core Laravel framework files, expose sensitive API keys or credentials in publicly accessible code, or commit `.env` files to version control.
